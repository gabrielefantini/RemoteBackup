...
bool Socket::vrfy() {
    ...
    // ricerca della cartella di backup

    std::string server_path=get_server_dir(std::string(name),std::string(dir));
    int res=setup_dir(server_path);
    if(res==0){
        //caso cartella creata

    }else{
        //caso cartella già presente
    }
